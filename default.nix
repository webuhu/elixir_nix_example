{ pkgs ? import ./pkg/_pkgs.nix, MIX_ENV ? "prod", release_name ? "example" }:
...
rec {
  ...
  # Needs `--relaxed-sandbox` if used without setting hash (impure fetch)
  mix_deps = pkgs.callPackage ./pkg/mix_deps_get.nix {
    inherit elixir MIX_PATH MIX_REBAR3 MIX_ENV LANG;
  ...
  # Needs `--relaxed-sandbox` if used without setting hash & hashed inputs (impure fetch)
